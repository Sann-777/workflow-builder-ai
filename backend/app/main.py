                    ...
                    messages=[
                        {
                            ...
                            "content": (
                                ...
                                "- Main flow: Place nodes horizontally with 250px spacing (x: 100, 350, 600, 850, etc.)\n"
                                "- Decision branches: When a decision node has multiple paths:\n"
                                "  * Success/Yes path: Continue on same y-level (y=100)\n"
                                "  * Failure/No path: Place 250px BELOW (y=350)\n"
                                "  * Alternative paths: Use y=600, y=850, etc.\n"
                                "- NEVER place two nodes at the same x,y coordinates\n"
                                "- Always maintain minimum 250px spacing between nodes\n"
                                "\n"
                                "EDGE CONNECTION RULES (CRITICAL):\n"
                                "- Edge structure: {id, source, target, sourceHandle, targetHandle, type: 'smoothstep'}\n"
                                "- For horizontal flow (left to right): sourceHandle='output-right', targetHandle='input-left'\n"
                                "- For vertical flow (top to bottom): sourceHandle='output-bottom', targetHandle='input-top'\n"
                                "- For decision nodes with 2 paths:\n"
                                "  * Yes/Success path: sourceHandle='output-yes' (connects to node on same level)\n"
                                "  * No/Failure path: sourceHandle='output-no' (connects to node below)\n"
                                "- Start nodes: Only have output handles (output-right, output-bottom)\n"
                                "- End nodes: Only have input handles (input-left, input-top)\n"
                                "- Process nodes: Have both input and output handles\n"
                                "- Always use proper handle IDs for clean, systematic connections\n"
                                "\n"
                                "WORKFLOW STRUCTURE:\n"
                                "- Start with exactly ONE 'start' node\n"
                                "- End with exactly ONE 'end' node\n"
                                "- All nodes must be connected (no orphan nodes)\n"
                                "- Decision nodes should have 2 outgoing edges (yes/no paths)\n"
                                "- Create logical, sequential flow from start to end\n"
                                "\n"
